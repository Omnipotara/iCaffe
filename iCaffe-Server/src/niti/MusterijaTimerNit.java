        ...
        while (!kraj) {
            try {
                ...
                if (preostalo.getSeconds() > 0) {
                    ...
                    // salje update SVAKE SEKUNDE
                    Kontroler.getInstance().posaljiVremeUpdateProdavcima(musterija);

                       
                }
                if (musterija.getPreostaloVreme().isZero() || musterija.getPreostaloVreme().isNegative()) {
                    Kontroler.getInstance().istekloVreme(musterija);
                    Kontroler.getInstance().obavestiProdavce(musterija);
                    Kontroler.getInstance().posaljiVremeUpdateProdavcima(musterija); // poslednji update
                    kraj = true;
                }
